...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            bs, _, ny, nx = map(int, x[i].shape)  # x[i](bs,self.no * self.na,20,20) to x[i](bs,self.na,20,20,self.no)
            bs = -1
            ...
            if not self.training:  # inference
                ...
                z.append(y.view(bs, self.na * ny * nx, self.no)) # z (list[P3_pred]): Torch.Size(b, n_anchors, self.no)
