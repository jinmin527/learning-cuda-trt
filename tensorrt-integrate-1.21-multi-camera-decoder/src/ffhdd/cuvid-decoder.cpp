...
namespace FFHDDecoder{
    ...
    public:
        bool create(bool bUseDeviceFrame, int gpu_id, cudaVideoCodec eCodec, bool bLowLatency = false,
                ...
                int maxWidth = 0, int maxHeight = 0, unsigned int clkRate = 1000, FrameType output_type = FrameType::BGR)
            {
        ...
        int decode(const uint8_t *pData, int nSize, uint64_t nTimestamp=0) override
        {
            m_nDecodedFrame = 0;
        ...
        int handlePictureDisplay(CUVIDPARSERDISPINFO *pDispInfo){
            ...
            if(m_output_type == FrameType::BGR || m_output_type == FrameType::RGB){
                if(m_pYUVFrame == 0){
                ...
                if(m_output_type == FrameType::BGR)
                    convert_nv12_to_bgr_invoker(y, uv, m_nWidth, m_nLumaHeight, m_nWidth, (uint8_t*)m_pBGRFrame, m_cuvidStream);
                else if(m_output_type == FrameType::RGB)
                    convert_nv12_to_rgb_invoker(y, uv, m_nWidth, m_nLumaHeight, m_nWidth, (uint8_t*)m_pBGRFrame, m_cuvidStream);
        ...
        int get_frame_bytes() override { assert(m_nWidth); 
            if(m_output_type == FrameType::BGR || m_output_type == FrameType::RGB){
        ...
        uint8_t* get_frame(uint64_t* pTimestamp = nullptr, unsigned int* pFrameIndex = nullptr) override{
    ...
    private:
        ...
        std::vector<uint64_t> m_vTimestamp;
        ...
        FrameType m_output_type = FrameType::BGR;
    };

    std::shared_ptr<CUVIDDecoder> create_cuvid_decoder(
        bool bUseDeviceFrame, IcudaVideoCodec eCodec, int max_cache, int gpu_id,
        const CropRect *pCropRect, const ResizeDim *pResizeDim, FrameType output_type){

        shared_ptr<CUVIDDecoderImpl> instance(new CUVIDDecoderImpl());
        if(!instance->create(bUseDeviceFrame, gpu_id, (cudaVideoCodec)eCodec, false, pCropRect, pResizeDim, max_cache, 0, 0, 1000, output_type))
            instance.reset();
        return instance;
    }
}; //FFHDDecoder