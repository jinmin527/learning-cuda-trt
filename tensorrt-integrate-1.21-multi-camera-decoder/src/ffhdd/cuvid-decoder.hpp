...
namespace FFHDDecoder{
    ...
    public:
        ...
        virtual uint8_t* get_frame(uint64_t* pTimestamp = nullptr, unsigned int* pFrameIndex = nullptr) = 0;
        virtual int decode(const uint8_t *pData, int nSize, uint64_t nTimestamp=0) = 0;
    ...
    std::shared_ptr<CUVIDDecoder> create_cuvid_decoder(
        ...
        const CropRect *crop_rect = nullptr, const ResizeDim *resize_dim = nullptr, FrameType output_type = FrameType::BGR
    );
}; // FFHDDecoder
