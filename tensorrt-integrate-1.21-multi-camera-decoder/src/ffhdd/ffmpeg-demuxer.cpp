...
namespace FFHDDemuxer{
    ...
    static inline bool check_ffmpeg_retvalue(int e, const char* call, int iLine, const char *szFile) {
        if (e < 0) {
            ...
            char buffer[1024];
            av_strerror(e, buffer, sizeof(buffer));

            SimpleLogger::__log_func(
                szFile, iLine, SimpleLogger::LogLevel::Error,
                "FFMPEGDemuxer error %s, code = 0x%X, %s", call, e, buffer
            );
            return false;
        }
        return true;
    ...
    public:
        bool open(const string& uri, bool auto_reboot = true, uint64_t timescale = 1000 /*Hz*/){
        ...
        bool demux(uint8_t **ppVideo, int *pnVideoBytes, uint64_t *pts = nullptr, bool *iskey_frame = nullptr) override{
            ...
            *ppVideo = nullptr;
            ...
            if (m_bMp4H264 || m_bMp4HEVC) {
                ...
                *pnVideoBytes = m_pktFiltered.size;
                local_pts = (uint64_t) (m_pktFiltered.pts * m_userTimeScale * m_timeBase);
            } else {
                ...
                local_pts = (uint64_t)(m_pkt.pts * m_userTimeScale * m_timeBase);
            }
    ...
    private:
        ...
        bool open(AVFormatContext *fmtc, uint64_t timeScale = 1000 /*Hz*/) {
            if (!fmtc) {
    ...
    private:
        ...
        uint64_t m_userTimeScale = 0; 
        uint8_t *m_pDataWithHeader = nullptr;
        unsigned int m_frameCount = 0;
        string uri_opened_;
        uint64_t time_scale_opened_ = 0;
