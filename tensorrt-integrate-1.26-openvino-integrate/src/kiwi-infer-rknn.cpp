...
namespace rknn{
    ...
    std::shared_ptr<kiwi::Infer> load_infer_from_memory(const void* pdata, size_t size, bool sync_mode){
        INFOE("Unimplement backend RKNN.");
        return nullptr;
    }

    std::shared_ptr<kiwi::Infer> load_infer(const std::string& file_or_data, bool sync_mode){
        INFOE("Unimplement backend RKNN.");
        return nullptr;
    }
